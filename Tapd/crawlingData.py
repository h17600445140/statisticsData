...
def crawlingData(driver,version,iteration,username,password):
    ...
    url = "https://www.tapd.cn/67410840/bugtrace/bugreports/my_view?" + "perpage=100&" + \
          "data[Filter][version_report][]=" + parse.quote(version)  + \
          "&data[Filter][iteration_id][]=" + str(iteration)
    ...
    while True:
        ...
        for bug_title, version, severity, priority, state, handler, creator, bug_createTime, bug_closeTime, bug_solveTime, bug_verificationTime, fixPeople in zip(bug_title, version, severity, priority, state, handler, creator, bug_createTime, bug_closeTime, bug_solveTime, bug_verificationTime, fixPeople):
            data_list.append({"bug标题":bug_title.text,
                              "发现版本":version.text,
                              "严重程度":severity.text,
                              "优先级":priority.text,
                              "状态":state.text,
                              "处理人":handler.text.split(';')[0],
                              "创建人":creator.text.strip(),
                              "创建时间":bug_createTime.text.split(' ')[0],
                              "关闭时间":bug_closeTime.text.split(' ')[0],
                              "解决时间":bug_solveTime.text.split(' ')[0],
                              "验证时间":bug_verificationTime.text.split(' ')[0],
                              "修复人":fixPeople.text.strip()})
